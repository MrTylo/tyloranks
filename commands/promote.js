...
exports.run = async (client, message, args) => {
  if (
    !message.member.roles.cache.some(role =>
      ["Ranking Permissions"].includes(role.name)
    )
  ) {
  ...
  if (
    Number(process.env.maximumRank) <= rankInGroup ||
    Number(process.env.maximumRank) <= rankInGroup + 1
  ) {
    return message.channel.send({
      embed: {
        color: 16733013,
  ...
  try {
    ...
    console.log(
      chalk.red("An error occured when running the promote command: " + err)
    );
    return message.channel.send({
      embed: {
        color: 16733013,
